...
const CargaDeDados: React.FC = () => {
  ...
  const handleDownloadAllPurchasedItems = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('purchased_items')
        .select('id, user_id, c_prod, descricao_do_produto, u_com, q_com, v_un_com, created_at, internal_product_name, invoice_id, item_sequence_number, x_fant, invoice_number, invoice_emission_date, is_manual_entry') // Listar explicitamente as colunas, removendo raw_xml_data
  ...
  const handleDownloadAllPurchasedItemsXml = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('purchased_items')
        .select('id, user_id, c_prod, descricao_do_produto, u_com, q_com, v_un_com, created_at, internal_product_name, invoice_id, item_sequence_number, x_fant, invoice_number, invoice_emission_date, is_manual_entry') // Removendo raw_xml_data
        .eq('user_id', user?.id)
        .order('created_at', { ascending: false });
