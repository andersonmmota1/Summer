...
export interface PurchasedItem { // Exportar a interface para uso no xml-exporter
  ...
  total_invoice_value?: number | null; // NOVO: Adicionado campo para o valor total da nota
}
...
const CargaDeDados: React.FC = () => {
      ...
      try {
        ...
        const formattedData = data.map((row: any) => {
          ...
          return {
            ...
            total_invoice_value: row.total_invoice_value || null, // NOVO: Adiciona o valor total da nota
          };
        });
  ...
  const handleDownloadAllPurchasedItems = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('purchased_items')
        .select('id, user_id, c_prod, descricao_do_produto, u_com, q_com, v_un_com, created_at, internal_product_name, invoice_id, item_sequence_number, x_fant, invoice_number, invoice_emission_date, is_manual_entry, total_invoice_value') // Listar explicitamente as colunas, incluindo total_invoice_value
      ...
      const headers = [
        ...
        'Entrada Manual',
        'Valor Total da Nota', // NOVO: Cabeçalho para o valor total da nota
      ];
      ...
      const formattedData = data.map(item => ({
        ...
        'Valor Total da Nota': item.total_invoice_value?.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || 'N/A', // NOVO: Inclui o valor total da nota
      }));
  ...
  const handleDownloadAllPurchasedItemsXml = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('purchased_items')
        .select('id, user_id, c_prod, descricao_do_produto, u_com, q_com, v_un_com, created_at, internal_product_name, invoice_id, item_sequence_number, x_fant, invoice_number, invoice_emission_date, is_manual_entry, total_invoice_value') // Incluindo total_invoice_value
        .eq('user_id', user?.id)
        .order('created_at', { ascending: false });
  ...
  const onSubmitManualEntry = async (values: ManualEntryFormValues) => {
    ...
    try {
      const formattedData = {
        ...
        total_invoice_value: null, // Entradas manuais não têm valor total da nota automaticamente
      };
